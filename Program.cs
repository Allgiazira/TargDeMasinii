    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                Console.WriteLine("A. Afisarea ultima masina introdusa");
...
//Lab 4 citirea argumentelor
        ...
        public static void AfisareMasina(TargMasini student)
        {
            string infoStudent = string.Format("Masina cu id-ul #{0} are specificatiile: Nume Vanzator: {1}. Nume Cumparator: {2}. Firma: {3}. Model: {4}. An Fabricatie: {5}." +
                "Culoarea: {6}. Optiuni: {7}. Data tranzactiei: {8}. Pret: {9}.",
                   student.GetIdMasina(),
                   student.GetNumeVanzator() ?? " NECUNOSCUT ",
                   student.GetNumeCumparator() ?? " NECUNOSCUT ",
                   student.GetFirma() ?? "NECUNOSCUT ",
                   student.GetModel() ?? " NECUNOSCUT ",
                   student.GetAnFab() ?? "NECUNOSCUT ",
                   student.GetCuloare() ?? "NECUNOSCUT ",
                   student.GetOptiuni() ?? "NECUNOSCUT ",
                   student.GetDataTranz() ?? "NECUNOSCUT ",
                   student.GetPret() ?? " NECUNOSCUT ");
...
//Lab 3 citire de la tastatura
        public static TargMasini CitireMasinaTastatura()
        {
            ...
            TargMasini student = new TargMasini(0, numevanzator, numecumparator, model, firma, an_fab, culoare, optiuni, data_tranz, pret);
